    ...
    if (keyTag === "tag:yaml.org,2002:merge") {
      if (Array.isArray(valueNode)) {
        for (
          ...
          this.mergeMappings(data, valueNode[index], overridableKeys);
        }
      } else {
        this.mergeMappings(
          data,
      ...
      if (
        ...
        Object.hasOwn(data, keyNode)
      ) {
        this.line = startLine || this.line;
        this.position = startPos || this.position;
        throw this.#createError("Cannot store mapping pair: duplicated key");
      }
      Object.defineProperty(data, keyNode, {
        value: valueNode,
        writable: true,
        enumerable: true,
        configurable: true,
  ...
  readPlainScalar(
    ...
    while (ch !== 0) {
      ...
      if (hasPendingContent) {
        ...
        data = writeFoldedLines(data, this.line - line);
  ...
  readSingleQuotedScalarData(
    ...
    while (ch !== 0) {
      if (ch === SINGLE_QUOTE) {
        ...
        data = writeFoldedLines(data, count);
        captureStart = captureEnd = this.position;
      } else if (
        this.position === this.lineStart &&
